window.onload = function () {
  ...
  function drawSmallTickMarks(options) {
    ...
    for (iTick = 10; iTick < 180; iTick += 20) {
      ...
      var innerTickX =
        gaugeOptions.radius - Math.cos(iTickRad) * gaugeOptions.radius;
      var innerTickY =
        gaugeOptions.radius - Math.sin(iTickRad) * gaugeOptions.radius;
  ...
  function drawTextMarkers(options) {
    ...
    for (iTick = 10; iTick < 180; iTick += options.divisions) {
      innerTickX =
        gaugeOptions.radius - Math.cos(degToRad(iTick)) * gaugeOptions.radius;
      innerTickY =
        gaugeOptions.radius - Math.sin(degToRad(iTick)) * gaugeOptions.radius;

      // Some cludging to center the values (TODO: Improve)
      if (iTick <= 10) {
        options.ctx.fillText(
          iTickToPrint,
          options.center.X - gaugeOptions.radius - 12 + innerTickX,
          gaugeOptions.center.Y - gaugeOptions.radius - 12 + innerTickY + 5
        );
      } else if (iTick < 50) {
        options.ctx.fillText(
          iTickToPrint,
          options.center.X - gaugeOptions.radius - 12 + innerTickX - 5,
          gaugeOptions.center.Y - gaugeOptions.radius - 12 + innerTickY + 5
        );
      } else if (iTick < 90) {
        options.ctx.fillText(
        ...
        options.ctx.fillText(
          iTickToPrint,
          options.center.X - gaugeOptions.radius - 12 + innerTickX + 4,
          gaugeOptions.center.Y - gaugeOptions.radius - 12 + innerTickY
        );
      } else if (iTick < 145) {
        options.ctx.fillText(
          iTickToPrint,
          options.center.X - gaugeOptions.radius - 12 + innerTickX + 10,
          gaugeOptions.center.Y - gaugeOptions.radius - 12 + innerTickY
        );
      } else {
        options.ctx.fillText(
          iTickToPrint,
          options.center.X - gaugeOptions.radius - 12 + innerTickX + 15,
          gaugeOptions.center.Y - gaugeOptions.radius - 12 + innerTickY + 5
        );
      }

      // MPH increase by 10 every 20 degrees
      iTickToPrint += options.divisions;
    }
  ...
  function convertSpeedToAngle(options) {
     ...
     * equivelant angle.
     */
    var iSpeed = options.speed / options.divisions,
  ...
  function drawNeedle(options) {
    ...
    var endNeedleX =
      gaugeOptions.radius - Math.cos(iSpeedAsAngleRad) * gaugeOptions.radius;
    var endNeedleY =
      gaugeOptions.radius - Math.sin(iSpeedAsAngleRad) * gaugeOptions.radius;
  ...
  function getSpeedFromServer() {
    $.get("http://localhost:3000/speed", ({ speed }) => {
      draw(speed);
    }).fail(() => {
  ...
  // request speed from server in every 500ms
  const myInterval = setInterval(getSpeedFromServer, 500);
};
